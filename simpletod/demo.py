...
from utils.multiwoz.nlp import normalize_for_sql
...
class MultiWozDB(object):
    ...
    hotel_info = ['name', 'area', 'internet', 'parking', 'phone', 'postcode', 'pricerange', 'stars', 'takesbookings',
                  'type', 'address']
    train_info = ["arriveBy", "day", "departure", "destination", "duration", "leaveAt", "price", "trainID"]
    restaurant_info = ["address", "area", "food", "id", "introduction", "name", "phone", "postcode", "pricerange",
                       "signature", "type"]
    attraction_info = ["address", "area", "entrance fee", "id", "name", "openhours", "phone", "postcode", "pricerange",
                       "type"]
    taxi_info = []
    database_keys = {
        'hotel': hotel_info,
    ...
    def queryResultVenues(self, domain, turn, real_belief=False):
        ...
        for key, val in items:
            if val == "" or val == "dontcare" or val == 'not mentioned' or val == "don't care" or val == "dont care" \
                    or val == "do n't care":
                pass
            if 'book' in key:
                pass
...
def lexicalize_train(delex_response, db_results, turn_beliefs, turn_domain):
    ...
    if 'from [value_place] to [value_place]' in delex_response:
        ...
        lex_response = lex_response.replace('from [value_place] to [value_place]',
                                            'from {} to {}'.format(departure, destination))
    ...
    if 'leaving [value_place] at [value_day]' in delex_response:
        departure = sample['departure']
        day = sample['day']
        lex_response = lex_response.replace('leaving [value_place] at [value_day]',
                                            'leaving {} at {}'.format(departure, day))

    if 'leaving [value_place] at [value_time]' in delex_response:
        leaveat = sample['leaveAt']
        departure = sample['departure']
        lex_response = lex_response.replace('leaving [value_place] at [value_time]',
                                            'leaving {} at {}'.format(departure, leaveat))
    if 'leaves [value_place] at [value_time]' in delex_response:
        leaveat = sample['leaveAt']
        departure = sample['departure']
        lex_response = lex_response.replace('leaves [value_place] at [value_time]',
                                            'leaves {} at {}'.format(departure, leaveat))
    ...
    if 'arrives in [value_place] at [value_time]' in delex_response:
        ...
        destination = sample['destination']
        lex_response = lex_response.replace('arrives in [value_place] at [value_time]',
                                            'arrives in {} at {}'.format(destination, arriveby))
