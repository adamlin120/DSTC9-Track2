...
class MultiWozEvaluator(BaseEvaluator):
    ...
    def _evaluateGeneratedDialogue(self, dialname, dial, goal, realDialogue, real_requestables, soft_acc=False):
        ...
        for domain in goal.keys():
            ...
            #         # if realDialogue['goal'][domain].has_key('reqt') and 'id' not in realDialogue['goal'][domain][
            #         'reqt']:
            ...
            # if id was not requested but train was found we dont want to override it to check if we booked the right
            # train
