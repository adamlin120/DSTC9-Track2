...
for i, dial_name in enumerate(lex_dict):
    ...
    for turn_id, (usr_turn, _) in enumerate(zip(user, system)):
        ...
        if USE_DB_SEARCH and not USE_ORACLE_BELIEF:  # generate belief, then get DB search results, then continue
    ...
    for turn, pred in enumerate(generated):
        ...
        if 'openai-gpt' in model_checkpoint:
            ...
            if 'dbsearch' in model_checkpoint or 'dbnmatch' in model_checkpoint or USE_DB_SEARCH or 'db' in \
                    model_checkpoint:
