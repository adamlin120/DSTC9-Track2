...
def convert_dbpointer_to_text(vect, goal, belief):
    ...
    for idx in range(4):
        ...
        if domain != 'train':
            ...
            if (domain == 'restaurant' and np.all(restaurant_book_vec == np.array([0, 1]))) or (
                    domain == 'hotel' and np.all(hotel_book_vec == np.array([0, 1]))):
...
def convert_dbpointer_to_text_nmatch(vect, goal, belief):
    ...
    for idx in range(4):
        ...
        if domain != 'train':
            ...
            # if (domain == 'restaurant' and np.all(restaurant_book_vec == np.array([0, 1]))) or (domain == 'hotel'
            # and np.all(hotel_book_vec == np.array([0, 1]))):
            #     # text.append('{} match{} booking=available'.format(domain, domain_match_text))
            #     text.append('{} match{}'.format(domain, domain_match_text))
            # else:
            #     text.append('{} match{} booking=not available'.format(domain, domain_match_text))
...
def one_hot_vector(num: int, domain: str, vector: np.ndarray):
...
def query_result(domain: str, turn_meta: Dict[str, Dict]) -> int:
    ...
    flag = True
    for key, val in turn_meta[domain]['semi'].items():
        if val == "" or val == "dont care" or val == 'not mentioned' or val == "don't care" or val == "dontcare" or \
                val == "do n't care":
            pass
        else:
            if flag:
...
def queryResultVenues(domain, turn, real_belief=False):
    ...
    for key, val in items:
        if val == "" or val == "dontcare" or val == 'not mentioned' or val == "don't care" or val == "dont care" or \
                val == "do n't care":
            pass
        else:
            if flag:
